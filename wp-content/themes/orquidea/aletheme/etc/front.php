...
function ale_enqueue_scripts() {
    ...
    wp_register_script( 'jquery.nicescroll.min', THEME_URL . '/js/libs/jquery.nicescroll.min.js', array( 'jquery' ), ALETHEME_THEME_VERSION, false );
    wp_register_script( 'jquery.mousewheel', THEME_URL . '/js/libs/jquery.mousewheel.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'mwheelIntent', THEME_URL . '/js/libs/mwheelIntent.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    ...
    wp_register_script( 'jquery.isotope.min', THEME_URL . '/js/libs/jquery.isotope.min.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'scrollable', THEME_URL . '/js/libs/scrollable.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'jquery.nivo.slider.pack', THEME_URL . '/js/libs/jquery.nivo.slider.pack.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    wp_register_script( 'jquery.fullscreen-min', THEME_URL . '/js/libs/jquery.fullscreen-min.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
    ...
    if(ale_get_option('sitecustomscrollbar')==1){
 ...
 * @param int $depth 
 */
function aletheme_comment($comment, $args, $depth) {
	$GLOBALS['comment'] = $comment; ?>
	<li <?php comment_class(); ?> data-comment-id="<?php echo $comment->comment_ID?>">
		<div id="comment-<?php comment_ID(); ?>">
			<header class="comment-author vcard cf">
				<?php printf(__('<cite class="fn">%s</cite>', 'aletheme'), get_comment_author_link()); ?> /
				<time datetime="<?php echo comment_date('c'); ?>"><?php printf(__('%1$s', 'aletheme'), get_comment_date('d M-Y'),  get_comment_time()); ?></time>
				<?php edit_comment_link(__('(Edit)', 'aletheme'), '', ''); ?>
			</header>
			<?php if ($comment->comment_approved == '0') : ?>
			<p><?php _e('Your comment is awaiting moderation.', 'aletheme'); ?></p>
			<?php endif; ?>
...
<<?php echo $tag ?> <?php comment_class(empty( $args['has_children'] ) ? '' : 'parent') ?> id="comment-<?php comment_ID() ?>">
    ...
    <div class="commentbody">
        ...
        <span class="date"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>">
            ...
            printf( __('%1$s at %2$s','aletheme'), get_comment_date(),  get_comment_time()) ?></a>
         /
...
function ale_footer() {
    ...
    <?php if(ale_get_option('comments_style')=='fb' and is_single()){ ?>
        <div id="fb-root"></div>
        ...
        <?php }
        if(ale_get_option('gallerycomments_style')=='fb' and get_post_type()=='gallery'){ ?>
